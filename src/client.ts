...
export class Pdffillr {
  ...
  constructor({
    ...
    this.logLevel = parseLogLevel(options.logLevel, 'ClientOptions.logLevel', this) ?? parseLogLevel(readEnv('PDFFILLR_LOG'), 'process.env[\'PDFFILLR_LOG\']', this) ?? defaultLogLevel;
  ...
  protected validateHeaders({ values, nulls }: NullableHeaders) {
    ...
    if (this.developerJwt && values.get('authorization')) {
      return;
    }
    if (nulls.has('authorization')) {
      return;
    ...
    throw new Error('Could not resolve authentication method. Expected either apiKey or developerJwt to be set. Or for one of the "Authorization" or "Authorization" headers to be explicitly omitted')
  }

  protected async authHeaders(opts: FinalRequestOptions, schemes: { apiKeyAuth?: boolean, developerSessionAuth?: boolean }): Promise<NullableHeaders | undefined> {
    return buildHeaders([schemes.apiKeyAuth ? await this.apiKeyAuth(opts) : null, schemes.developerSessionAuth ? await this.developerSessionAuth(opts) : null]);
  }
  ...
  protected async developerSessionAuth(opts: FinalRequestOptions): Promise<NullableHeaders | undefined> {
    if (this.developerJwt == null) {
      return undefined;
    }
